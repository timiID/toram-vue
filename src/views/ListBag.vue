  ...
  <div :class="['bg-transparent py-0 px-2 md:px-4 font-sans relative z-10 transition-all duration-500', isDark ? 'text-white' : 'text-slate-900']">
    ...
    <div class="max-w-9xl mx-auto flex justify-center md:justify-end mb-2 sticky top-0 z-50">
      <div class="flex flex-col items-center gap-1 pt-2">
        <span class="text-[12px] font-[1000] tracking-[0.4em] text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-purple-500 to-red-500 uppercase text-center w-full opacity-90">
          LANGUAGE THIS PAGE
        </span>
        ...
        <div class="inline-flex p-1 rounded-xl bg-white/10 backdrop-blur-xl border-2 border-white/20 shadow-2xl">
          <button v-for="l in ['id', 'en']" :key="l" @click="lang = l"
            :class="['px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all duration-300 flex items-center gap-3 group/btn',
            lang === l 
            ...
            <img 
              ...
              class="w-5 h-3.5 object-cover rounded-sm shadow-sm group-hover/btn:scale-110 transition-transform"
              alt="flag"
            />
    ...
    <div class="max-w-7xl mx-auto space-y-4 md:space-y-6">
      ...
      <div :class="['relative p-6 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] border-[4px] overflow-hidden backdrop-blur-2xl transition-all duration-700 shadow-2xl',
        ...
        <div class="absolute top-0 right-0 h-full w-1/3 hidden lg:block pointer-events-none">
          <img src="/images/Bag-Toram.png" class="w-full h-full object-contain object-right scale-105" />
        </div>
        <div class="relative z-10 text-center md:text-left animate-in slide-in-from-left duration-700">
          <h2 :class="['text-3xl md:text-6xl font-black italic uppercase leading-none tracking-tighter', isDark ? 'text-white' : 'text-slate-900']">
            {{ t.title }}<br>
          ...
          <div class="flex items-center justify-center md:justify-start gap-3 mt-2">
              ...
              <img src="/images/logo.png" class="h-6 md:h-10 w-auto object-contain" />
              <span class="font-[1000] italic uppercase text-2xl md:text-3xl">TIMI <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-purple-500 to-red-500">DB</span></span>
          </div> 
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <div v-for="(s, i) in stats" :key="i" 
              :class="['p-4 md:p-5 rounded-[1.5rem] border-[3px] shadow-xl transition-all hover:scale-105 bg-gradient-to-br text-white animate-in zoom-in', s.grad]"
              :style="{ transitionDelay: i * 100 + 'ms' }">
              <div class="text-[9px] font-black opacity-80 uppercase">{{ s.label }}</div>
              <div class="text-2xl md:text-4xl font-black italic mt-0.5">{{ s.val }}</div>
          </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="relative">
            <label class="text-[9px] font-black uppercase ml-4 mb-1 block text-indigo-500 tracking-widest">{{ t.searchLabel }}</label>
            <div class="relative group">
                <input v-model="search" type="text" :placeholder="t.searchPlaceholder" 
                    :class="['w-full h-12 pl-12 pr-4 rounded-xl border-[3px] font-bold outline-none transition-all',
        ...
        <div class="relative">
            <label class="text-[9px] font-black uppercase ml-4 mb-1 block text-indigo-500 tracking-widest">{{ t.catLabel }}</label>
            <div class="relative group">
                <select v-model="selectedKelas"
                    :class="['w-full h-12 pl-12 pr-10 rounded-xl border-[3px] font-bold outline-none appearance-none cursor-pointer transition-all',
                    isDark ? 'bg-white/5 border-white/10 text-white focus:border-indigo-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-indigo-600 focus:bg-white']">
                    <option value="All Class">{{ t.allClass }}</option>
                    <option v-for="opt in ['Mob', 'Miniboss', 'Boss', 'Spina']" :key="opt" :value="opt">{{ opt }}</option>
                </select>
                <div class="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-500 pointer-events-none">
                ...
                <div class="absolute right-4 top-1/2 -translate-y-1/2 text-indigo-500 pointer-events-none group-hover:translate-y-[-40%] transition-transform">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
                </div>
            </div>
        </div>

        <div class="relative">
            <label class="text-[9px] font-black uppercase ml-4 mb-1 block text-indigo-500 tracking-widest">{{ t.slotLabel }}</label>
            <div class="relative group">
                ...
                <div class="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-500 pointer-events-none">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                </div>
                <div class="absolute right-4 top-1/2 -translate-y-1/2 text-indigo-500 pointer-events-none group-hover:translate-y-[-40%] transition-transform">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
                </div>
      ...
      <div :class="['border-[3px] rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl transition-all', isDark ? 'border-lime-400 bg-black/60' : 'border-lime-500 bg-white/80 shadow-lime-100']">
          <div class="hidden lg:block overflow-x-auto no-scrollbar">
              <table class="w-full min-w-[1000px] border-collapse">
                  <thead>
                      <tr :class="['uppercase text-[9px] font-black tracking-widest border-b-[2px]', isDark ? 'bg-white/5 text-lime-400 border-lime-500/30' : 'bg-slate-50 text-lime-700 border-lime-200']">
                          <th class="p-4 w-[10%] text-center">{{ t.table.slot }}</th>
                          <th class="p-4 w-[20%] text-left">{{ t.table.name }}</th>
                          <th class="p-4 w-[10%] text-center">{{ t.table.need }}</th>
                          <th class="p-4 w-[15%] text-center">{{ t.table.cost }}</th>
                          <th class="p-4 w-[15%] text-left">{{ t.table.src }}</th>
                          <th class="p-4 w-[10%] text-center">{{ t.table.type }}</th>
                          <th class="p-4 w-[20%] text-left">{{ t.table.loc }}</th>
                      </tr>
                  </thead>
                  <tbody :class="['divide-y-[1px]', isDark ? 'divide-white/10' : 'divide-slate-200']">
                      <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-indigo-500/5 transition-all group">
                          <td class="p-4 text-center font-black text-blue-500 italic text-2xl">{{ item.slot }}</td>
                          <td class="p-4 font-black uppercase text-sm leading-tight">{{ lang === 'id' ? (item.nama_item_id || item.nama_item_en) : (item.nama_item_en || item.nama_item_id) }}</td>
                          <td class="p-4 text-center bg-black/5"><span class="font-black italic text-lime-600 text-3xl">{{ formatQty(item.jumlah) }}</span></td>
                          <td class="p-4 text-center font-black text-lg text-amber-600">{{ item.spina > 0 ? Number(item.spina).toLocaleString() : '-' }}</td>
                          <td class="p-4 text-[10px] font-bold uppercase opacity-60">
                              {{ lang === 'id' ? (item.monster_id || item.monster_en || '-') : (item.monster_en || item.monster_id || '-') }}
                          </td>
                          <td class="p-4 text-center"><span :class="getBadgeClass(item.kelas_mob)" class="scale-90">{{ item.kelas_mob }}</span></td>
                          <td class="p-4 font-black text-sm italic text-blue-600 uppercase">{{ lang === 'id' ? (item.lokasi_id || item.lokasi_en) : (item.lokasi_en || item.lokasi_id) }}</td>
                      </tr>
                  </tbody>
              </table>
          </div>
          ...
          <div class="lg:hidden divide-y-[2px] divide-slate-200/10">
              <div v-for="item in paginatedItems" :key="item.id" class="p-4 space-y-3">
                  <div class="flex justify-between items-center">
                      <div class="text-[9px] font-black text-blue-500 uppercase">SLOT {{ item.slot }}</div>
                      <div :class="getBadgeClass(item.kelas_mob)" class="!min-w-fit px-2 py-0.5 text-[8px]">{{ item.kelas_mob }}</div>
                  </div>
                  <h3 class="font-black uppercase text-base leading-tight">{{ lang === 'id' ? (item.nama_item_id || item.nama_item_en) : (item.nama_item_en || item.nama_item_id) }}</h3>
                  <div :class="['grid grid-cols-2 gap-2 border-2 p-3 rounded-xl', isDark ? 'border-white/10 bg-white/5' : 'border-slate-100 bg-slate-50']">
                      <div><span class="text-[8px] font-black text-lime-600 block uppercase">Need</span><span class="text-2xl font-black italic text-lime-600">{{ formatQty(item.jumlah) }}</span></div>
                      <div class="border-l-[1px] pl-3 border-slate-300/30"><span class="text-[8px] font-black text-amber-600 block uppercase">Cost</span><span class="text-base font-black mt-1">{{ item.spina > 0 ? Number(item.spina).toLocaleString() : '-' }}</span></div>
                  </div>
              </div>
          </div>
      </div>

      <div class="flex flex-col gap-6 justify-center items-center pb-10">
          <div v-if="totalPages > 1" class="flex flex-wrap justify-center items-center gap-1.5">
              <button @click="currentPage--" :disabled="currentPage === 1" class="h-10 w-10 rounded-lg border-2 font-black border-blue-600 disabled:opacity-20">«</button>
              <button v-for="p in displayedPages" :key="p" @click="typeof p === 'number' ? currentPage = p : null" 
                :class="['h-10 w-10 rounded-lg border-2 font-black transition-all text-sm', p === currentPage ? 'bg-blue-600 text-white shadow-md' : 'text-blue-600 border-blue-100 hover:border-blue-500']">
                  {{ p }}
              </button>
              <button @click="currentPage++" :disabled="currentPage === totalPages" class="h-10 w-10 rounded-lg border-2 font-black border-blue-600 disabled:opacity-20">»</button>
          </div>
          <button @click="toggleShowAll" class="h-12 px-10 rounded-xl border-[3px] border-indigo-400 bg-indigo-600 text-white font-black uppercase italic text-xs tracking-wider hover:scale-105 active:scale-95 transition-all shadow-lg">
